...
class Predictor(BasePredictor):
    def setup(self):
        ...
        self.device = "cuda"
        self.model = whisperx.load_model("large-v2", self.device, language="vi", compute_type=compute_type)
        self.alignment_model, self.metadata = whisperx.load_align_model(language_code="vi", device=self.device)
